...
/* Memoria compartida mecanismo IPC
        -se comparte una region de memoria
        -se llama "chared memory objects" objetos de memoria ocmpartida
        --En Linux --> /dev/shm
        -se borran al reiniciar la pc
        -sincronizacion en los programas
